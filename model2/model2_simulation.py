...
for i in range(len(times) - 1):
    ...
    if sum(APtimes < (times[i] - dt)) < sum(
            APtimes < times[i]):  # meaning that the t+dt is reaching the timing of a new AP
    ...
    IG = gap_leak(V[i], E(R, T, F, Krest, Ki), 1, Gg, Ki, myelinK[i], Ni, Ni)
    Ig[i]=sum(IG)

    myelinK[i + 1] -= flux(Ia_e[0] + Ia_s[0] + Ik_s[i] + Ik_e[i] + IH_s[0] + IH_e[0] + IL_e[0] + IL_s[0]+IG[0], V_myelin, F,
                           dt)
    submyelinK[i + 1] += flux(Ia_s[0] + Ik_s[i] + IH_s[0] + IL_s[0], V_submyelin, F, dt)

    V[i + 1] = V_t_dt_1(V[i], Cm, dt, Ik_s[i] + Ik_e[i] + Ih_s[i] + Ih_e[i] + sum(Ia_s) + sum(Ia_e) + Il_e[i] + Il_s[i]+Ig[i])
...
fig,ax=mpl.subplots()
mpl.plot(times, Ig*1e12,color='grey',label="cell-cell interface")
ax.set_xlabel("Time (s)")
ax.set_ylabel("I (pA)")
mpl.title("Leak current  over time ")
mpl.legend()
